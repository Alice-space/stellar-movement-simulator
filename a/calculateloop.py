...
same notes in Mem) '''
    ...
    if judgelist != []:
        ...
        while loopstate == True:
            ...
            loopstate = judgement
            judgelist = templs
        """ The following part deletes the objects that are to be emerged andcreates new emerged objects. """
    ...
    if judgelist == []:
        ...
        if neworder == True:
            ...
            pass
        for objnum in [k for k in range(n) if k not in emergingobjects]:
            templs = [tempt] + vel[objnum] + cor[objnum]
            objects[objnum].dataIN(templs)
        return False
