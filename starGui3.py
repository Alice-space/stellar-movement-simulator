...
@LastEditTime : 2019-12-26 01:12:16
'''
...
Del = []
MyStar=[]
Type = ['Eplanet', 'Jplanet', 'starG', 'starO']
cardTexture=[[Type[j]+'/'+chr(j+97)+str(i+1)+'.jpg' for i in range(5)] for j in range(4)]
triggerstate = False
...
def Species():  #four basic cards to create object
    ...
    for i in range(4):
        typeframe[i] = DirectFrame(parent=StarMenucanvas,
                                   ...
                                   pos=(0, 0, 1 - 0.27 * i),
                                   relief=1,
                                   frameTexture=path + 'speciesTexture/'+Type[i]+
                                   '.jpg',
                                   image_scale=0.25,
                                   text=typeText[i],
    ...
    if button == 1:    #'Done'
        ...
        if legitimate:
            ...
            if newStar == False:
                ...
                canvasSize = (0, 0.99, StarMenu['canvasSize'][2] - 0.675, 1)
            leftframe.destroy()
...
def attachTexture(objtype):
    typeNumber=Type.index(objtype)
    a=random.randint(0,4)
    return path+cardTexture[typeNumber][a]

def generateCard(obj):
    global underChangeState
    starNumber = MyStar.index(obj)
    openCount, closedCount = 0, 0
    info = '\1roman\1mass: %d   kg\nradius: %d   km\2' % (obj.mass, obj.radius)
    for obj in MyStar[0:starNumber]:
        if obj.switch == True:
            openCount = openCount + 1
        else:
            closedCount = closedCount + 1
    obj.texture2D=attachTexture(obj.objtype)
    ...
    myButton = DirectCheckButton(parent=card,
                                 boxRelief=1,
                                 boxBorder=0,
                                 boxImage=path+'starImg/'+obj.objtype+'.jpg',
                                 scale=0.174,
                                 pos=(0.378, 0, -0.174),
                                 relief=0,
                                 command=Change,
    ...
    if obj.switch == True:
        ...
        for i in range(3):
            velo[i] = DirectEntry(
                parent=infoCard,
                initialText='{:' '>8}'.format(int(v[i])) +
                '{:.3f}'.format(v[i] - int(v[i])[1:])[1:],
                scale=.035,
        ...
        for j in range(3):
            posi[2 - j] = DirectEntry(
                parent=infoCard,
                initialText='{:' '>8}'.format(int(p[i])) +
                '{:.3f}'.format(p[i] - int(p[i])[1:])[1:],
                scale=.035,
